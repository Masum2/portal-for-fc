...
export const HistoricalUpdate = () => {
  ...
  // Mock Historical Records List
  const [records] = useState([
    {
      id: 1,
      tableName: "APS_Referral",
      recordId: 125,
      updatedBy: "Masum Billah",
      updatedOn: "2026-07-30 10:15 AM",
      jsonData: {
        id: 125,
        clientName: "John Doe",
        status: "Approved",
        referralFee: 150.00,
        metadata: { verifiedBySystem: true, riskScore: "Low" },
        notes: "Client verification successfully completed via national ID validation."
      }
    },
    {
      id: 2,
      ...
      recordId: 211,
      updatedBy: "Anik Aich",
      updatedOn: "2026-07-28 09:40 AM",
      jsonData: {
        id: 211,
        title: "Q3 Strategy Expansion",
        category: "Internal Operations",
        published: false,
        contributors: ["Anik Aich", "Sarah Jenkins"],
        metrics: { targetReach: 50000, budgetAllocatedUSD: 12500 }
      }
    },
    {
      id: 3,
      tableName: "APS_Client",
      recordId: 984,
      updatedBy: "Tanvir Ahmed",
      updatedOn: "2026-07-25 04:20 PM",
      jsonData: {
        clientId: 984,
        organization: "Apex Solutions Ltd",
        tier: "Enterprise",
        activeContracts: 4,
        supportSettings: { prioritySupport: true, dedicatedManager: "Masum Billah" }
      }
    }
  ]);
  ...
  const handleSearch = () => {
    console.log("Fetching data with filters:", { selectedTable, selectedUser, fromDate, toDate });
    setCurrentPage(1);
    // Real implementation: fetch(`api/historical-updates?table=${selectedTable}&user=${selectedUser}&from=${fromDate}&to=${toDate}`)
  };
    ...
    <div className="min-h-screen bg-slate-50/70 p-6 md:p-10 font-sans text-slate-800 antialiased">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Admin Panel &gt; Audit Logs &gt; View all system historical update logs and inspect JSON snapshots.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-xs border border-slate-200/80 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          {/* Table Name (Searchable / Select Dropdown) */}
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all cursor-pointer"
            >
          ...
          {/* Updated By (Searchable / Select Dropdown) */}
          <div>
            <label className="text-xs font-semibold text-slate-600 mb-1.5 block">
              Updated By
            </label>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all cursor-pointer"
            />
          </div>
          ...
          <div className="flex items-end gap-2">
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-medium text-sm px-4 py-2.5 rounded-xl w-full transition-colors shadow-xs flex items-center justify-center gap-1.5 cursor-pointer"
            >
      ...
      {/* 4. Historical Updates Table Section */}
      <div className="bg-white rounded-2xl shadow-xs border border-slate-200/80 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200 text-xs font-bold uppercase tracking-wider text-slate-500">
                ...
                <th className="px-6 py-3.5">Updated On</th>
              </tr>
            </thead>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center px-6 py-4 border-t border-slate-100 gap-4">
          ...
          <div className="inline-flex items-center gap-1.5">
            ...
            <span className="px-3 py-1 bg-blue-600 text-white rounded-lg text-xs font-bold shadow-xs">
              {currentPage}
            </span>
            <button
      ...
      {activeJson && (
        <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-800 text-sm tracking-wide">
                  Structured JSON Inspector (appsettings style)
                </h3>
              </div>
              <button
            ...
            <div className="p-6 bg-slate-900 text-emerald-400 font-mono text-xs overflow-y-auto flex-1 leading-relaxed shadow-inner">
